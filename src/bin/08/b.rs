...
fn main() {
    ...
    let mut last_two_paired: Option<(Point3D<usize>, Point3D<usize>)> = None;
    // The solution from the first half is effectively Kruskal's algorithm if
    // we run to completion, except we don't track which edges were used.
    while !sqdist_heap.is_empty() {
        let pair = sqdist_heap.pop().expect("Not enough pairs");
        let id0 = groupings.get(&pair.0);
        let id1 = groupings.get(&pair.1);
        let mut did_pair = true;
        match (id0, id1) {
            (Some(id), None) => {
                let id = *id;
            ...
            (Some(id0), Some(id1)) => {
                ...
                if id0 == id1 {
                    ...
                    groups[id1].clear();
                    groups[id0].extend(items.into_iter());
                }
            }
            (None, None) => {
                let id = groups.len();
        ...
        if did_pair {
            last_two_paired = Some((pair.0, pair.1));
        }
    }

    if let Some((p0, p1)) = last_two_paired {
        println!(
            "Product of X coordinates of last two paired: {}",
            p0.x * p1.x
        );
    } else {
        println!("Somehow did not pair any points");
    }
}
